...
cart is created and changed → checkout validates current menu/availability
and commercial inputs → `OrderCreated` commits an immutable order snapshot →
payment and restaurant acceptance proceed as separate lifecycles → staff
accepts, prepares, marks ready and completes pickup/fulfillment → customer
receives status.
...
settlement is a distinct recorded fact.
...
`nekurama.chatgpt.md:L22696-L23080`; Raw mapped-flow anchors `T208`
(`5ec0497b-76d4-4977-bc62-179f63f51edc`) and `T223`
(`3d343ca8-c4e9-4a06-9735-27a084e41064`); `brd.md`,
...
`experience-and-channels.md`, `validation.md`.
...
`product-definition.md`, `business-model.md`.
...
ESCALATE | RECONCILE`; reconciliation returns through a valid domain
transition.
...
alerts → event-derived projections update asynchronously → analytics pipeline
aggregates business events for activation, order success, fulfillment, staff
adoption, support, repeat use, economics and willingness-to-pay decisions →
pilot review uses the recorded evidence.
...
`validation.md`, `experience-and-channels.md`.
